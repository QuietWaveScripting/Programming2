...
const string PARAMS_ERROR = "Error: wrong number of parameters.",
...
int main() {
    ...
    while ( true ) {
        ...
        // DONE: Implement command execution here!
        //fetch the correct command from vector
        Command comm;
        bool command_found = false;
        for (Command const &c : COMMANDS){
            // check if names match
            if(c.str == str_to_upper(command_to_be_executed)){
                  comm = c;
                  command_found = true;
            }
        }
        ...
        //check amount of parameter size
        if(pieces.size() != comm.parameter_number + 1){
            cout << PARAMS_ERROR << endl;
            continue;
        }

        // first check if exit command
        if(comm.is_exit){
        ...
        // check operand types
        string param1_str = pieces.at(1),
                param2_str = pieces.at(2);
        double param1, param2;

        if(!string_to_double(param1_str,param1) || !string_to_double(param2_str,param2)){
            cout << NON_NUMBER_OPERAND_ERROR << endl;
            continue;
        }

        // execute command
        cout << comm.action(param1, param2) << endl;
